...
const Card = ({ product, isBestRate, onClick, className, isLoading, selectedProduct }: {
    ...
    const { type, name, bestRate, id } = product || {};
    ...
    return (
       <div key={id} className={`product-card m-8 border border-gray-300 rounded-2xl p-5 sm:p-10 ${className || ''}`}>
            ...
            <div className="text-3xl font-bold mb-10">{bestRate}%</div>
            
            <button
                disabled={isLoading && product?.id !== selectedProduct}
                className="inline-flex items-center border border-gray-400 px-6 py-3 rounded-xl hover:bg-gray-100 transition" 
                onClick={onClick}>
                {(isLoading && product?.id === selectedProduct) && (
                    <>
                ...
                {(!isLoading || product?.id !== selectedProduct) && (<>Select this product</>)}
            </button>
        </div>
    );
};
